...
void menu::setData()
{
    ...
    query.prepare("Select *   from menu where  ID_STOCK= :id");
...
void menu::on_pushButton_43_clicked()
{
    if (getModifier()==0)
    {
       ...
       // QString descrip_elem=ui->lineEdit_8->text();
        int prix_element=ui->lineEdit_10->text().toInt()  ;
        int id_stock=1;
    menu_c m(id_stock,prix_element,nom_elem,type_elem,photo_path);
    ...
    else if (getModifier()==1)
    {
       ...
       menu_c m(id_elem,prix_element,nom_elem,type_elem,photo_path);
