...
bool menu_c::modifier_menu(int id_elem)
{
    ...
    query.prepare("UPDATE menu SET NOM_ELEM=:nom_elem,TYPE_ELEM=:type_elem,DESCRIP_ELEM=:descrip_elem , PRIX_ELEMENT=:prix_element where ID_ELEM= :id_elem");

    query.bindValue(":nom_elem",nom_elem);
    query.bindValue(":type_elem",type_elem);
     query.bindValue(":decrip_elem",descrip_elem);
